...
The Site/CI install step must also run `npm --prefix functions/attachment-processor ci --include=dev` before the root `pnpm build`, because that command verifies both packages. This does not bundle Function dependencies into the Site; the two API route output traces contain no attachment parser, AWS attachment client or Function package files.
